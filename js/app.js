...
async function refreshProjectList() {
  ...
  if (!projects || projects.length === 0) {
    container.innerHTML = '<div class="empty-state"><div class="empty-icon"><img src="icons/新建.svg" style="width:48px;height:48px;"></div><p>暂无项目，点击上方新建</p></div>';
    return;
  }
  ...
  container.innerHTML = projects.map(p => {
    ...
    return `<div class="card" style="cursor:pointer;${isActive ? 'border-color:var(--primary);' : ''} ${isCompleted ? 'opacity:0.85;' : ''}" onclick="openProject('${p.id}')">
      ...
      <div class="flex-between">
        <span style="font-size:12px;color:var(--text-dim);"><img src="icons/进度.svg" class="icon-inline"> ${isCompleted ? '已竣工 (100%)' : '进度 ' + progress + '%'}</span>
        <div>
...
function renderDailyOverview(project, allLogs, todayLog) {
  ...
  el.innerHTML = `
    ...
    <div style="font-size:11px;color:var(--text-dim);margin-top:6px;padding-top:6px;border-top:1px solid var(--border);display:flex;justify-content:space-between;">
      <span><img src="icons/跟进.svg" class="icon-inline"> 计划${totalPlanned}项 · 完成${totalDone}项</span>
      <span><img src="icons/图表-柱状图.svg" class="icon-inline"> 跟进率${trackedRate}% (${daysWithData}/${elapsed}天)</span>
    </div>
  `;
...
async function refreshPhotos() {
  ...
  if (filtered.length === 0) {
    grid.innerHTML = '<div class="empty-state"><div class="empty-icon"><img src="icons/相机.svg" style="width:48px;height:48px;"></div><p>暂无照片</p></div>';
    return;
  }
...
async function showReworkDetail(id) {
  ...
  document.getElementById('reworkDetailContent').innerHTML = `
    ...
    ${photosHtml ? '<div class="mt-12"><div style="font-size:13px;color:var(--text-dim);margin-bottom:6px;"><img src="icons/相机.svg" class="icon-inline"> 照片:</div><div style="display:flex;gap:6px;flex-wrap:wrap;">' + photosHtml + '</div></div>' : ''}
...
function exportReportTxt() {
  ...
  showToast('<img src="icons/保存.svg" class="icon-inline"> 已导出TXT');
}
...
async function refreshCompletion() {
  ...
  if (isOverview) {
    ...
    html += `<div class="card">
      <div class="btn-group">
        <button class="btn btn-sm btn-secondary" onclick="uploadCompletionFile('image')"><img src="icons/相机.svg" class="icon-inline"> 上传照片</button>
